...
YOUTUBE_TEMPLATE = 'rtmps://a.rtmp.youtube.com:443/live2/'
KICK_TEMPLATE = 'rtmp://ingest.kick.com/live/'
...
def bg_loop(cfg):
    ...
    while stream_active:
        ...
        for src in all_sources:
            ...
            if not src:
                continue
            wr(f'Checking source: {src}')
            hls = get_hls_url(src)
            if not hls:
                wr(f'No live stream at {src}')
                continue
            ...
            p.wait()
            rc = p.returncode
            wr(f'Stream ended (exit {rc})')
            ffmpeg_proc = None
            if not stream_active:
                break
            wr(f'Source ended, trying next...')
            time.sleep(2)

        if not stream_active:
            break
        if not found:
            wr('No live sources found, retrying in 30s...')
            time.sleep(30)
...
def preview():
    ...
    hls = get_hls_url(cfg['source_url'])
    if not hls:
        return jsonify({'ok': False, 'error': 'Cannot resolve URL — check yt-dlp logs'}), 400
    return jsonify({
        'ok': True,
        'hls': hls,
        'source': cfg['source_url'],
...
<title>Gaming Restream Panel</title>
...
<body>
<div class="container">
<h1>🎮 Gaming Restream Panel <small>v1</small></h1>
  ...
  <div class="card">
    ...
    <form id="configForm">
      <div class="form-group">
        <label>Streamer URL (primary source)</label>
        <input type="url" name="source_url" id="source_url" placeholder="https://www.twitch.tv/streamer">
      </div>
      <div class="form-row">
        <div class="form-group">
          <label>Output Mode</label>
          <select name="output_mode" id="output_mode" onchange="toggleOutputMode()">
      ...
      <div class="form-group">
        ...
        <input type="text" name="output_url" id="output_url" placeholder="rtmps://... or rtmp://...">
      </div>
      <div class="form-group">
        <label>Backup Streamers (one per line)</label>
        <textarea name="backup_list" id="backup_list" placeholder="https://www.twitch.tv/backup1"></textarea>
      </div>
      <div class="form-group">
...
function toggleOutputMode() {
  ...
  const url = document.getElementById('output_url');
  if (mode === 'youtube') url.placeholder = 'rtmps://a.rtmp.youtube.com:443/live2/STREAM_KEY';
  else if (mode === 'kick') url.placeholder = 'rtmp://ingest.kick.com/live/STREAM_KEY';
  else url.placeholder = 'rtmp:// or rtmps://...';
}
...
function testSource() {
  ...
  el.textContent = 'Checking if source is live...';
